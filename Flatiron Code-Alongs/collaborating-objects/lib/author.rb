class Author
    attr_accessor :name

    def initialize()
        @stories = [] # has many stories interface
    end

    def stories # has many stories interface
        @stories.dup.freeze # frozen object, prevents changes from the outside
    end

    def add_story(story) # has many stories interface
        # interface over Array#push to implement custom error
        raise AssociationTypeMismatchError, "#{story.class} received, Story expected." if !story.is_a?(Story)
        @stories << story
        story.author = self unless story.author == self
    end

    def bibliography()
        self.stories.collect{|s| s.name} # You need all stories to be instances of Story because they must respond to methods.
        # self.stories.collect(&:name) #=> Symbol to Proc, returns the same as the expression above. This is considered "too magical" by most Rubyists.
    end

    def categories # has many categories through stories
        self.stories.collect{|s| s.category}.uniq
    end    
end